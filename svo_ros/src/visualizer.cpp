...
Visualizer::Visualizer()
    : pnh_("~"), trace_id_(0), img_pub_level_(vk::getParam<int>("svo/publish_img_pyr_level", 0)),
      img_pub_nth_(vk::getParam<int>("svo/publish_every_nth_img", 1)), dense_pub_nth_(vk::getParam<int>("svo/publish_every_nth_dense_input", 1)),
...
void Visualizer::publishMinimal(const cv::Mat &img, const FramePtr &frame, const FrameHandlerMono &slam, const double timestamp)
{
    ++trace_id_;
    ...
    if (img_pub_nth_ > 0 && trace_id_ % img_pub_nth_ == 0 && pub_images_.getNumSubscribers() > 0)
    {
        ...
        if (slam.stage() == FrameHandlerBase::STAGE_SECOND_FRAME)
        {
            ...
            for (vector<cv::Point2f>::const_iterator it_ref = px_ref.begin(), it_cur = px_cur.begin(); it_ref != px_ref.end(); ++it_ref, ++it_cur)
                cv::line(img_rgb, cv::Point2f(it_cur->x / scale, it_cur->y / scale), cv::Point2f(it_ref->x / scale, it_ref->y / scale),
                         cv::Scalar(0, 255, 0), 2);
        }
        ...
        if (img_pub_level_ == 0)
        {
            for (Features::iterator it = frame->fts_.begin(); it != frame->fts_.end(); ++it)
            {
                if ((*it)->type == Feature::EDGELET)
                    cv::line(img_rgb, cv::Point2f((*it)->px[0] + 3 * (*it)->grad[1], (*it)->px[1] - 3 * (*it)->grad[0]),
                             cv::Point2f((*it)->px[0] - 3 * (*it)->grad[1], (*it)->px[1] + 3 * (*it)->grad[0]), cv::Scalar(255, 0, 255), 2);
                else  // point size 5x5
                    cv::rectangle(img_rgb, cv::Point2f((*it)->px[0] - 2, (*it)->px[1] - 2), cv::Point2f((*it)->px[0] + 2, (*it)->px[1] + 2),
        ...
        {  // point size 3x3
            for (Features::iterator it = frame->fts_.begin(); it != frame->fts_.end(); ++it)
                cv::rectangle(img_rgb, cv::Point2f((*it)->px[0] / scale - 1, (*it)->px[1] / scale - 1),
                              cv::Point2f((*it)->px[0] / scale + 1, (*it)->px[1] / scale + 1), cv::Scalar(0, 255, 0), CV_FILLED);
        }
        else
        {  // point size 1x1
            for (Features::iterator it = frame->fts_.begin(); it != frame->fts_.end(); ++it)
...
void Visualizer::visualizeMarkers(const FramePtr &frame, const set<FramePtr> &core_kfs, const Map &map)
{
    if (frame == NULL)
        return;

    vk::output_helper::publishTfTransform(frame->T_f_w_ * T_world_from_vision_.inverse(), ros::Time(frame->timestamp_), "cam_pos", "world", br_);

    if (pub_frames_.getNumSubscribers() > 0 || pub_points_.getNumSubscribers() > 0)
    {
        vk::output_helper::publishCameraMarker(pub_frames_, "cam_pos", "cams", ros::Time(frame->timestamp_), 1, 0.3, Vector3d(0., 0., 1.));
        vk::output_helper::publishPointMarker(pub_points_, T_world_from_vision_ * frame->pos(), "trajectory", ros::Time::now(), trace_id_, 0, 0.006,
                                              Vector3d(0., 0., 0.5));
...
void Visualizer::displayKeyframeWithMps(const FramePtr &frame, int ts)
{
    ...
    vk::output_helper::publishFrameMarker(pub_frames_, T_world_cam.rotation_matrix(), T_world_cam.translation(), "kfs", ros::Time::now(),
    ...
    for (Features::iterator it = frame->fts_.begin(); it != frame->fts_.end(); ++it)
    {
        ...
        vk::output_helper::publishPointMarker(pub_points_, T_world_from_vision_ * (*it)->point->pos_, "pts", ros::Time::now(), (*it)->point->id_, 0,
                                              0.005, Vector3d(1.0, 0., 1.0), publish_points_display_time_);
...
void Visualizer::exportToDense(const FramePtr &frame)
{
    ...
    if (frame != NULL && dense_pub_nth_ > 0 && trace_id_ % dense_pub_nth_ == 0 && pub_dense_.getNumSubscribers() > 0)
    {
