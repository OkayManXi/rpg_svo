...
BenchmarkNode::BenchmarkNode(ros::NodeHandle &nh) : vo_(NULL), frame_count_(0), img_noise_sigma_(vk::getParam<double>("svo/dataset_noise_sigma", 0.0))
{
    // Create Camera
...
void BenchmarkNode::tracePose(const SE3 &T_w_f, const double timestamp)
{
    ...
    trace_est_pose_ << p.x() << " " << p.y() << " " << p.z() << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << std::endl;
}
...
void BenchmarkNode::runBlenderBenchmark(const std::string &dataset_dir)
{
    ...
    for (auto it = dataset.begin(); it != dataset.end() && ros::ok(); ++it, ++frame_count_)
    {
        ...
        cv::Mat depthmap;
        vk::blender_utils::loadBlenderDepthmap(dataset_dir + "/depth/" + it->image_name_ + "_0.depth", *cam_, depthmap);
        ...
        if (frame_count_ == 0)
        {
            ...
            svo::feature_detection::FastDetector detector(cam_->width(), cam_->height(), svo::Config::gridSize(), svo::Config::nPyrLevels());
