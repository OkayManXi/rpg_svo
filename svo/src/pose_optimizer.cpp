                         ...
                         double &error_init, double &error_final, size_t &num_obs)
{
    ...
    for (size_t iter = 0; iter < n_iter; iter++)
    {
        ...
        {
            if (verbose)
                std::cout << "it " << iter << "\t FAILURE \t new_chi2 = " << new_chi2 << std::endl;
        ...
        chi2 = new_chi2;
        if (verbose)
            std::cout << "it " << iter << "\t Success \t new_chi2 = " << new_chi2 << "\t norm(dT) = " << vk::norm_max(dT) << std::endl;
    ...
    if (verbose)
        std::cout << "n deleted obs = " << n_deleted_refs << "\t scale = " << estimated_scale << "\t error init = " << error_init
